	...
	#region ContextMenu
		...
		private void contextMenuStrip1_Opening( object sender, CancelEventArgs e )
		{
			...
			if ( _masterPane != null && menuStrip != null )
			{
				...
				if ( _isShowContextMenu )
				{
					...
					item.Name = "copy";
					item.Tag = "copy";
                    item.Text = "复制";     // _resourceManager.GetString("copy");
					...
					item.Name = "save_as";
					item.Tag = "save_as";
                    item.Text = "另存图表...";     //_resourceManager.GetString( "save_as" );
					...
					item.Tag = "page_setup";
                    item.Text = "页面设置...";     //_resourceManager.GetString( "page_setup" );
					...
					item.Name = "print";
					item.Tag = "print";
                    item.Text = "打印...";     // _resourceManager.GetString("print");
					...
					item.Name = "show_val";
					item.Tag = "show_val";
                    item.Text = "显示节点数值";     //  _resourceManager.GetString( "show_val" );
					...
					if ( pane == null || pane.ZoomStack.IsEmpty )
                        menuStr = "还原缩放";     // _resourceManager.GetString( "unzoom" );
					else
					{
						switch ( pane.ZoomStack.Top.Type )
						{
							...
							case ZoomState.StateType.Pan:
                                menuStr = "还原移动";     //  _resourceManager.GetString( "unpan" );
								break;
							case ZoomState.StateType.Scroll:
                                menuStr = "还原卷动";     // _resourceManager.GetString( "unscroll" );
								break;
						}
					}
					...
					item.Name = "undo_all";
					item.Tag = "undo_all";
                    menuStr = "还原缩放/移动";       // _resourceManager.GetString("undo_all");
					item.Text = menuStr;
                    ...
                    menuStr = "恢复默认大小";       // _resourceManager.GetString( "set_default" );
					item.Text = menuStr;
		...
		public void Copy( bool isShowMessage )
		{
			if ( _masterPane != null )
			{
				...
				if ( isShowMessage )
				{
                    string str = "复制到剪贴板";        // _resourceManager.GetString("copied_to_clip");
